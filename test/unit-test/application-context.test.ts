...
test("ApplicationContext::storeState save null", (t) => {
  ...
  assert.deepEqual(readObj.arr, [1, "moo", 54.3]);
});
...
test("ApplicationContext::storeState save array", (t) => {
  ...
  assert.partialDeepStrictEqual(readObj, [1, "moo", 54.3]);
  assert.partialDeepStrictEqual(readObj, [{name: "test", value: 567}]);
});
